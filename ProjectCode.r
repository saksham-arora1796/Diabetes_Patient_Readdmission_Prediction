...
#Boosted tree Model #Saksham Arora
#gbm1 <- gbm(readmitted~ ., data= train_df,
#                distribution="bernoulli", n.trees=100, shrinkage=0.02, interaction.depth=3,
#                bag.fraction = .5, train.fraction = 1, n.minobsinnode = 50, cv.folds = 10,
#                keep.data=TRUE, verbose=FALSE)

#gbm1
#gbm1_yHat = predict(gbmModel, X_test, n.trees=100)

#gbmPR = postResample(pred=gbm1_yHat, obs=y_test)
#gbmPR
...
gbm.pred <- predict(gbmFit, X_test)
#PredBinary<- as.factor(ifelse(gbm.pred>0.7,1,0))
#train_df$readmitted<- as.factor(train_df$readmitted)
#gbm.pred <- merge(gbm.pred,  gbmFit$bestTune)

gbmFit$pred <- merge(gbmFit$pred,  gbmFit$bestTune)
gbmCM <- confusionMatrix(gbmFit, norm = "none")
gbmCM

gbmRoc <- roc(response = gbmFit$pred$X_test,
